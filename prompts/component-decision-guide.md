...
| ดู illustration empty state | `tbt-svg` + `tbt-button` | 11 built-ins: empty/search/success/error/warning/draft/no-access/no-data/no-results/maintenance/inbox-zero |
| ดูรูป/ชื่อย่อผู้ใช้ | `tbt-avatar` (+ `tbt-avatar-group` สำหรับหลายคน +N) | initials fallback + สีจากชื่อ + status dot |
| ดู empty state ในกล่อง/section | `tbt-empty-state` | icon + title + description + action slot (พร้อมใช้กว่า `tbt-svg` ดิบ) |
| ดู icon | `tbt-icon` | 80+ ERP alias (save/approve/reject/invoice…) + raw Tabler name |
| ดู tag / chip (ลบได้) | `tbt-chip` | removable, variant — filter / label |
| ดู timeline เหตุการณ์ | `tbt-timeline` | ลำดับเวลาแนวตั้ง + dot/variant |
| แบ่งหน้า (standalone) | `tbt-pagination` | ใช้เมื่อไม่ได้ใช้ `paginate` ในตัว `tbt-table` |
| เลือก 1 จากตัวเลือกน้อย (เห็นทุกตัว) | `tbt-radio` | radio group — แทน dropdown เมื่อ ≤5 ตัวเลือก |
| ดูข้อมูลลำดับชั้น (ผังบัญชี/BOM/หมวด) | `tbt-tree` | expand/collapse + select |
...
| Tabs / Subtabs | `tbt-subtab` (in-form) หรือ `tbt-tabs` + `tbt-tabs-panel` (ARIA-compliant) — Teibto เรียก in-form tabs ว่า **"Subtabs"** (terminology) |
